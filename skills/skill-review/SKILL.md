...
name: skill-review
description: |
  Production-ready comprehensive audit skill for claude-skills repository. Use when reviewing skills for updates, investigating documented issues, verifying before marketplace submission, or checking package version currency. Performs systematic 15-phase audit covering standards compliance, dependency verification, API validation, cross-file consistency, progressive disclosure review, and issue categorization. Enforces official Anthropic standards including YAML validation (1,024-char descriptions, 64-char names), SKILL.md line limits (<500), third-person style, and multi-model testing. Auto-fixes straightforward issues; asks user only for architectural decisions or breaking changes. Outputs severity-classified results (🔴 Critical / 🟡 High / 🟠 Medium / 🟢 Low) with evidence citations and detailed remediation plans. Production-tested on better-auth v2.0.0 audit (6 critical issues found, 665 incorrect lines removed, 1,931 correct lines added). See references/audit-methodology.md for complete methodology details.
