...
```
claude-skills/
├── .agents/                      # Codex CLI surface (generated)
│   └── plugins/
│       └── marketplace.json      # Codex marketplace (from generate-codex-manifests.sh)
├── .claude-plugin/               # Plugin marketplace
│   └── marketplace.json          # 145 plugins (generated by generate-marketplace.sh)
├── .githooks/                    # Git hooks (pre-commit, pre-push)
├── .github/                      # CI workflows, issue templates, CODEOWNERS
├── docs/                         # Documentation
│   ├── archive/                  # Historical records
│   ├── getting-started/          # Onboarding guides
│   ├── guides/                   # Contribution, best practices, marketplace ops
│   ├── reference/                # Standards, categorization, common mistakes
│   ├── security-audit/           # Audit reports and findings
│   └── validation/               # JSON schema validation docs
├── schemas/                      # JSON schemas (marketplace, plugin, codex variants)
├── scripts/                      # Automation (8 scripts + lib/)
│   ├── sync-plugins.sh           # ⭐ Single entry point
│   ├── generate-marketplace.sh   # Marketplace + MARKETPLACE.md generation
│   ├── generate-codex-manifests.sh # Codex CLI manifests
│   ├── validate-frontmatter.sh   # SKILL.md frontmatter validation (CI)
│   ├── validate-json-schemas.sh  # ajv schema validation (CI)
│   ├── check-versions.sh         # Verify dependency versions (manual)
│   ├── review-skill.sh           # Skill review automation (manual)
│   └── baseline-audit-all.sh     # Baseline validation (manual)
├── plugins/                      # ⭐ 145 production plugins
│   ├── cloudflare-*/             # 21 Cloudflare plugins
│   ├── ai-*/                     # AI/ML plugins
│   ├── api-*/                    # API plugins
│   └── ...                       # Run: ls plugins/
...
- Most plugins contain 1 skill; some contain multiple (bun: 27, cloudflare-workers: 10, nuxt-v4: 4) — skills are auto-discovered from each plugin's `skills/` directory
...
`sync-plugins.sh` uses a **single global version** for all plugins. The version is read from `marketplace.json` → `.metadata.version` and stamped onto every `plugin.json` and marketplace entry. There is **no per-plugin versioning**.
