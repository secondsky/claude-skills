...
# Run all JavaScript tests (can also use bun test:js if bun is configured)
pnpm test:js
...
- **Module resolution errors**: Run `pnpm install` (or `bun install` if using bun) in the `client/admin` directory
...
- WooCommerce uses pnpm; for other projects, prefer bun
